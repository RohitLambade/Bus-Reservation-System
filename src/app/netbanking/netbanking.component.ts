...
export class NetbankingComponent implements OnInit {
  ...
  constructor(private bser:BusService,private router:Router,private service:CommonServiceService) {
 ...
 seats:any;
  ...
  saveData(data:any)
  {
    console.log(this.service.getusertype())
    if(this.service.getusertype()){
      console.log(this.service.getusertype())
      this.router.navigate(['registered'])
    }
    else{
    this.router.navigate(['unreg']);
  }
 ...
 for(let i=0;i<this.seats.length;i++)
      ...
      this.bser.addTransaction(this.trans).subscribe(
        data=>{
          ...
          this.book.schedule_id=this.scheduleId;
          for(let i=0;i<this.seats.length;i++)
          {
